...
| Backend | The implementation/configuration name for a Trade Wind — `deepinfra` (GLM-5.2 via pi, default), `claude` (Claude Code), or `codex` (Codex). Registered in `ship/backends.json`, active choice per role in each charter's `.ship/backend.json`; switch with the `ship` command (`ship/bin/ship`, e.g. `ship <charter> <role> <backend-name>`). Switching is next-spawn-only, manual or reactive-auto (on a detected rate-limit signal) |
